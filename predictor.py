...
class Predictor:
    def __init__(self, model, job_type: str, input_date: str, input_distance: float | None):
        self.model = model
    ...
    def arima_pipeline(self, forecast_range):
        ...
        df=pd.read_csv("df_ts_rolled_resampled_15min.csv", index_col="tpep_pickup_datetime")[["mean_bill"]]
        ...
        y_test=y_test[y_test.index<=pd.to_datetime(input_date)+pd.Timedelta(f'{forecast_range}h')]
